...
- **潜空间 1.5 倍**：按用户选择的分辨率完成首采，使用 H3 3D 潜空间放大模型直接放大约 1.5 倍，再完成低 Sigma 尾段精修；不再先缩小到 75%，细节基础更完整，但显存占用会高于旧方式。
